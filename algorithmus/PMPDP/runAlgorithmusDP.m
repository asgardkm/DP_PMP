...
batEngMax       = fahrparams.batEngMax;
...
crsSpdVec   = fzg_array.geaRat(gea) * velVec / (fahrparams.whlDrr);
...
vehFrcSlp = fahrparams.vehMas * 9.81 * sin(slopeVec);
% rolling resistance
vehFrcRol = fahrparams.whlRosResCof*fahrparams.vehMas * 9.81 * cos(slopeVec);
% air resistance
vehFrcDrg = fahrparams.drgCof * velVec.^2;
...
whlFrc  = accVec*fahrparams.vehMas + vehFrcSlp + vehFrcRol + vehFrcDrg;
...
whlTrq = whlFrc*fahrparams.whlDrr;
...
if whlTrq < 0
    crsTrq = whlTrq / fzg_array_struct.geaRat(gea) * fahrparams.geaEfy;
else
    crsTrq = whlTrq / fzg_array_struct.geaRat(gea) / fahrparams.geaEfy;
end
...
iceTrqMax = fzg_array.iceTrqMaxCof(1) * crsSpdVec.^2 ...
    + fzg_array.iceTrqMaxCof(2) * crsSpdVec ...
    + fzg_array.iceTrqMaxCof(3);

% minimales Moment des Verbrennungsmotors berechnen
%   calculating mimimum ICE moment
iceTrqMin = fzg_array.iceTrqMinCof(1) * crsSpdVec.^2 ...
    + fzg_array.iceTrqMinCof(2) * crsSpdVec ...
    + fzg_array.iceTrqMinCof(3);

%% electric motor max torque
%   max torque that the electric motor can provide - from interpolation
emoTrqMaxPos = ...
    interp1q(fzg_array.emoSpdMgd(1,:)',fzg_array.emoTrqMax_emoSpd,crsSpdVec);
...
emoTrqMinPos = ...
    interp1q(fzg_array.emoSpdMgd(1,:)',fzg_array.emoTrqMin_emoSpd,crsSpdVec);
...
for i = 1 : length(batPwrMax)
    batPwrMax(i) = codegen_interp2(fzg_array.emoSpdMgd,fzg_array.emoTrqMgd,...
        fzg_array.emoPwr_emoSpd_emoTrq,crsSpdVec(i),emoTrqMax(i)) + batPwrAux;
end

for i = 1 : length(batPwrMin)
    batPwrMin(i) = codegen_interp2(fzg_array.emoSpdMgd,fzg_array.emoTrqMgd,...
        fzg_array.emoPwr_emoSpd_emoTrq,crsSpdVec(i),emoTrqMin(i)) + batPwrAux;
end
