...
crsTrqOptMat    = inf(length(timVec), length(batEndInxVec));
...
if ~tst_scalar_struct.useGeaSta
    ...
    [...
        ...
        emoTrq1Mat(:, batEndInx_counter),        ...
        emoTrq2Mat(:, batEndInx_counter),       ...
...
for tim = 1 : length(timVec)
   ...
   crsTrqOptMat(tim, :) = crsTrqMat(tim, geaStaMat(tim, :));
...
% yString = 'Torque [N*m]';
% plotGrad(timVec, emoTrq2Mat, batEndInxVec, titleString, xString, yString);
